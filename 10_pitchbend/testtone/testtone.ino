...
// 音階から周期を求める関数(ファインチューニング(32で半音上がる)付き)
unsigned long tone2interv(int tone_height, int fine){
  if(tone_height == 128){
    return 0;
  }
  int th = tone_height;
  ...
  // fn補正のかかったピッチを返す
  int interv_low = 1000000 / (int)(toneheight[th % 12] * pow(2, th / 12 - 5)) / 2;
  int interv_high = 1000000 / (int)(toneheight[(th + 1) % 12] * pow(2, (th + 1) / 12 - 5)) / 2;
  return (interv_low * (32 - fn) / 32 + interv_high * fn / 32);
}
...
void loop() {
  ...
  while(Serial.available() > 0){
    ...
    if(midi_indata >= 0x80 && midi_indata <= 0xF7){
        ...
        if(midi_stat_ch < CH_COUNT){
          if(midi_data_index % 2 == 1){
            ...
            if(midi_data_queue[midi_data_index] != 0){
              ...
              interv[midi_stat_ch] = tone2interv(height[midi_stat_ch], pitchbend[midi_stat_ch]);
              lasttime[midi_stat_ch] = micros();
            }else{
              // 実質ノートオフ
              if(midi_data_queue[midi_data_index - 1] == height[midi_stat_ch]){
      ...
      }else if(midi_stat_type == 14){
        // ピッチベンドのとき
        if(midi_stat_ch < CH_COUNT){
          if(midi_data_index % 2 == 1){
            // データを2バイト受け取った→反映しにいく
            // ピッチベンドの上位バイトは後から送られてくる
            pitchbend[midi_stat_ch] = midi_data_queue[midi_data_index] - 64;
            interv[midi_stat_ch] = tone2interv(height[midi_stat_ch], pitchbend[midi_stat_ch]);
          }
        }
      }
